    ...
    async with AsyncBaseChain() as chain:
        ...
        for pool in pools:
            pools_data.append({
                ...
                "reserve0": {"amount": str(pool.reserve0.amount)},
                "reserve1": {"amount": str(pool.reserve1.amount)},
                "pool_fee": pool.pool_fee,
                "is_cl": pool.is_cl,
                "is_stable": pool.is_stable,
                "factory": pool.factory
            })
        redis_client.set("pools", json.dumps(pools_data), ex=600)  # Cache for 10 minutes
...
def scheduled_job():
    asyncio.run(cache_pools())

# Fetch pools from Redis
async def get_pools_from_cache():
    cached_pools = redis_client.get("pools")
    if cached_pools:
        return json.loads(cached_pools)
    else:
        await cache_pools()  # Fetch and cache if not found
        return json.loads(redis_client.get("pools"))
